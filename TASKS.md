# Project Tasks

## Backlog
- [ ] **Task 002** (#8): Create "Services" section using reusable Card components.
- [ ] **Task 003** (#9): Implement "Our Impact" (Statistics) section.
- [ ] **Task 004** (#10): Add "Contact Us" section with a form.
- [ ] **Task 005** (#11): Enhance "Hero" section with an image and better layout.
- [ ] **Task 006** (#12): Ensure full mobile responsiveness across all sections.
- [ ] **Task 007** (#13): Add "Partners" or "Supported By" section (Makerere University, etc.).

## In Progress

## Completed
- [x] **Task 002** (#8): Create "Services" section using reusable Card components.
- [x] **Task 001** (#7): Implement Navigation Bar (Header) component.
- [x] **Task 000**: Initialize project documentation and workflow.
- [x] Initial project structure setup.
